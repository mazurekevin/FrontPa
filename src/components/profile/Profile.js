...
function Profile() {
    ...
    const fetchPosts= async()=>{
        const postResponse = await axios.get('http://141.94.245.122:8081/api/posts/user/'+name);
        setPosts(postResponse.data);

    }

    const fetchCheckFollow = async()=>{
         return await axios.post("http://141.94.245.122:8081/api/follow/checkFollow",{
            id : 2,
    ...
    const fetchFollowed= async()=>{
        const followedResponse = await axios.get('http://141.94.245.122:8081/api/follow/followed/'+name);
        setFollowed(followedResponse.data);

    }
    const fetchFollower= async()=>{
        const followerResponse = await axios.get('http://141.94.245.122:8081/api/follow/follower/'+name);
    ...
    const toggleFunction = async ()=>{
        if(!checkFollow){
            ...
            const createFollow = await axios.post("http://141.94.245.122:8081/api/follow",{
                followerUserName: JSON.parse(token).username,
                followedUserName: name
            });
        }else{
            await axios.post("http://141.94.245.122:8081/api/follow/deleteFollow",{
                id : 2,
                followerUserName: JSON.parse(token).username,
                followedUserName: name
            });
